package com.example.fan.its_trip.progress;
...
import com.example.fan.its_trip.BuildConfig;
import com.example.fan.its_trip.R;
import com.example.fan.its_trip.utils.Constant;
import com.example.fan.its_trip.utils.MiscUtil;
...
public class CircleProgress extends View {
    ...
    private void drawArc(Canvas canvas) {
        ...
        canvas.drawArc(mRectF, currentAngle, mSweepAngle - currentAngle + 6, false, mBgArcPaint);
        // 第一个参数 oval 为 RectF 类型，即圆弧显示区域
        // startAngle 和 sweepAngle  均为 float 类型，分别表示圆弧起始角度和圆弧度数
        // 3点钟方向为0度，顺时针递增
        // 如果 startAngle < 0 或者 > 360,则相当于 startAngle % 360
        // useCenter:如果为True时，在绘制圆弧时将圆心包括在内，通常用来绘制扇形
        canvas.drawArc(mRectF, 6, currentAngle, false, mArcPaint);
        canvas.restore();
    }
