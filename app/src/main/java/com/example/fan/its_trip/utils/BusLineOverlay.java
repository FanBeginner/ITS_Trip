...
import com.example.fan.its_trip.activity.BusLineSearchActivity;
...
public class BusLineOverlay extends OverlayManager {
    ...
    public boolean onBusStationClick(int index) {
        if (mBusLineResult.getStations() != null
            ...
            BusLineSearchActivity.nodeIndex=index;

            TextView popupText = new TextView(App.appContext);
            ...
            // 弹出泡泡
            popupText.setText(mBusLineResult.getStations().get(index).getTitle());
            mBaiduMap.showInfoWindow(new InfoWindow(popupText, mBusLineResult.getStations()
                    .get(index).getLocation(), 10));
